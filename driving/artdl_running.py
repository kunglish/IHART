...
def artdl(num, model_type, compare_type, hash_type="None", k1 = 10, k2 = 5):
    ...
    for i in range(num):
        ...
        list = [i, Type]
        start = time.time()
        F_meatures = art_feature_distance(model_type, originalDataset, noiseDataset, labels, vggnet, model, compare_type)
        list.append(F_meatures)
        list.append(time.time() - start)
        print(list)
        worksheet.append(list)

    filename = compare_type +"ART_" + str(k1) + "_" + str(k2) + "_driving_" + str(model_type) +"_output.xlsx"
